...
domains = isofunc.load_domain_mappings(path_pfam_gc, path_pfam_names, has_eval=False)
...
# map domains directly to orfs
# map domain mapped to ref, and domain (also hmmer) mapped to alt
for symbol, gene in gd.items():
    for orf in gene.orfs:
        tmp_orf = orf
        domain_infos = domains[orf.ensp]
...
# helper functions for code below
def get_pattern_blocks_and_strings(dom_alnrs):
    rna_cat_str = ''.join([alnr.alnb.cat for alnr in dom_alnrs])
    rna_cat_block = get_pattern_block(rna_cat_str)
    prot_cat_str = ''.join([alnr.alnpb.cat for alnr in dom_alnrs])
    prot_cat_block = get_pattern_block(prot_cat_str)
    return rna_cat_block, prot_cat_block, prot_cat_str

def get_pattern_block(cat_chain):
    """Get the pattern block. e.g., MMMMMSSSSM -> MSM"""
    return ''.join([x[0] for x in re.split('(M+)', cat_chain) if x])

def get_substitution_info_if_exists(prot_cat_block, dom_alnrs):
    """If domain whole AA substitution, grab subst.-specific info. for write-out."""
    extra = ['-'] * 5
    if prot_cat_block == 'S':
        extra = get_substitution_block_specific_info(dom_alnrs)
    return extra

def get_substitution_block_specific_info(dom_alnrs):
    """Pull out AA substition block-specific info."""
    alnpb = dom_alnrs[0].alnpb
    aa1 = alnpb.aa1.replace('-', '')
    aa2 = alnpb.aa2.replace('-', '')
    len_aa1, len_aa2 = len(aa1), len(aa2)
...
with open('a_domain_per_res_prot_block_change.txt', 'w') as ofile:
    ofile.write('ref_orf\talt_orf\tdom_name\trna_cmp_pattern\tprot_cmp_pattern\t'
                'prot_cmp_string\tsub_aa1\tsub_aa2\tsub_len_aa1\tsub_len_aa2\tsub_len_diff\t'
                'sub_perc_diff\n')
    for grps in all_grps:
        for grp in grps:
            grp.enter()
            ...
            for ref_featf in ref_orf.doms:
                ...
                dom_alnrs = alnf.get_subset_aln_chain(ref_featf.first.res.aln, ref_featf.last.res.aln)
                # get pattern blocks
                rna_cat_block, prot_cat_block, prot_cat_str = get_pattern_blocks_and_strings(dom_alnrs)
                extra = get_substitution_info_if_exists(prot_cat_block, dom_alnrs)
                odata = [ref_orf.name, alt_orf.name, ref_featf.desc, rna_cat_block, prot_cat_block, prot_cat_str] + extra
...
# write out all paired domains in ref/alt pairs, compare and print-out lengths
