...
def all_other_gc_orfs_of_gene(gc_gd, symbol, anchor_orf):
...
# write out list of genes where
with open('h_list_of_genes_with_dbd_written_out.tsv', 'w') as ofile:
    # list of genes where the DBD% was attempted written out
    for gene, domains_mapped in non_zf_domains.items():
        if len(domains_mapped) >= 1:
            ofile.write(gene + '\n')
...
for flank_status in ['with_flank', 'no_flank']:
    ...
    with open('h_GC_affect_of_mapped_uniprot_dbds_{}.tsv'.format(flank_status), 'w') as ofile:
        ...
        for symbol in genes:
            ...
            for enst, dom_infos in non_zf_domains[symbol].items():
                ...
                for other_orf in all_other_gc_orfs_of_gene(gc_gd, symbol, anchor_orf):
                    # align anchor orf to all other GC orfs of a gene, create an aln object
