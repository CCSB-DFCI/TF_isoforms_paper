...
def all_genes_with_same_strand(gen_objs):
    strands = set()
    for g in gen_objs:
...
def grab_gen_objs_from_orfs(orfs):
    """Get the set of gen_objs from set of orfs."""
    gen_objs = set()
    for orf in orfs:
        gen_objs.add(orf.gene)
    return gen_objs
...
def compress_introns_and_set_relative_orf_exon_and_cds_coords(gen_objs, orfs_to_plot, intron_spacing):
...
def find_and_set_subtle_splicing_status(orfs_to_plot, threshold=15):
    ...
    for cds in cdss:
        ...
        st_delta = None # init. to nothing, set to smallest delta when comparing cds start to all other coords in orf group
        en_delta = None
        for coord in coords:
            st, en = coord
...
def verify_all_orfs_of_same_strand(gen_objs):
...
def get_orf_color(orf):
    ...
    col_cats = {'GC':'steelblue', 'CL':'mediumseagreen', 'PB':'indianred',
                '4K':'mediumpurple', '6KOC':'mediumpurple'}
    try:
...
def render_iso_image(orfs_to_plot, mode='all', dname='x_isoimages',
                     has_muts=False, intron_spacing=30, comp=300, spacing=0.5,
    ...
    Input:
        ...
        has_muts - flag to plot muts and alter track spacing to accommodate mutation lollipops
    ...
    max_x, min_y = 0, 0 # track abs figsize (to correctly proportion output plot)
    fig, ax = plt.subplots()
    line = 0 # initial track position (y-axis)

    # mut parameters
    plotted = [] # list holding previous mut positions (to avoid overlap of lollipops)
    dis_acro_dict = {} # disease acronym dictionary acro -> descri.
    mut_size, mut_adjust = 0.03, 0.25

    # determine if input is a list of orfs or an isoform_matrix
    if is_orfs_to_plot_a_list_or_set(orfs_to_plot):
        orfs_to_plot = list(orfs_to_plot) # if orf list is set, conver to list
        mat_obj = None
    else:
        mat_obj, orfs_to_plot = extract_orfs_from_isoform_matrix(orfs_to_plot)
    ...
    cpad = 0.1 # padding between spaces in colony plots
    orf_bbox_right = 0 # furthest right position to start plotting colonies
    for orf in sort_orfs_by_plot_order(orfs_to_plot):
        ...
        orf_bbox_right = update_figure_range(orf_bbox_right, intron_end) + cpad



    # plot gene info and header lines
    repr_orf = get_repr_orf(orfs_to_plot)
    plt.text(sp[0], line, repr_orf.gene, fontweight='bold', va='top') # write-out gene descr.
    line -= spacing/2.0
    plt.text(sp[0], line, 'GC-isoname') # write-out column name
    ...
    if mat_obj:
        tmp_right_coord = orf_bbox_right # track pos. of partners
        repr_orf = orfs_to_plot[0] # assume that the repr orf has exact same ppis as all other orfs
        mat_gene = repr_orf.gene.name
        ordered_ppis = create_ordered_list_of_orfid_ppi_pairs(repr_orf, mat_gene)
        for genename, orfname in extract_genenames_and_orfnames_from_ordered_ppi_list(ordered_ppis):
            label = genename + ' ' + orfname
            plt.text(tmp_right_coord, header_line, label, rotation=90, va='bottom')
            tmp_right_coord += cpad * 2

    # if isoform_matrix object passed in, plot colony image or colored blocks
    if mat_obj:
        line_tmp = line + cpad
        for orf in sort_orfs_by_plot_order(orfs_to_plot):
            right_tmp = orf_bbox_right
            for orfid, ppi in create_ordered_list_of_orfid_ppi_pairs(orf, mat_gene):
                # line_tmp += cpad*3
                for i, retest in enumerate(ppi.retests):
                    if mat_squares == 'colony':
                        img_path = retest.at.image_path
                        im_array, extent = generate_image_array_and_plot_coords(img_path, right_tmp, line_tmp, cpad)
                        ax.imshow(im_array, extent=extent, interpolation='none')
                    elif mat_squares == 'color':
                        if i == 1:
                            scolor = determine_ppi_result_square_color(retest.ppi) # square color
                            if scolor:
                                ax.add_patch(mpatches.Rectangle([right_tmp, line_tmp], 0.1, 0.1, ec='k', fc=scolor))
                            else:
                                ax.text(right_tmp, line_tmp, 'NA')
                    line_tmp -= cpad
                right_tmp += cpad*2
                max_x = update_figure_range(max_x, right_tmp)
                line_tmp += cpad*3
            line_tmp = line_tmp - spacing
    ...
    for orf in sort_orfs_by_plot_order(orfs_to_plot):
        ...
        label = orf_name + ' ' + orfid
        plt.text(sp[0], line, label, va='bottom') # write-out orfname
        intron_start, intron_end, intron_line = get_intron_plot_coordinates(orf, comp, line, height)
        ...
        for exon in orf.exons:
            ...
            if exon.cds:
                ...
                if delta_start: plt.text(x, y+height*2, delta_start, va='bottom', ha='left', size='x-small')
                if delta_end: plt.text(x+blength, y+height*2, delta_end, va='bottom', ha='right', size='x-small')
            ...
            if has_muts:
                for pos in exon.chain:
                    ...
                    for mut in pos.muts:
                        ...
                        ax.add_patch(mpatches.Circle([mstart, mline+mut_space], mut_size, fc=col, ec='k', lw=1, zorder=2))
                        ax.add_line(mlines.Line2D([mstart, mstart], [line+height*1.5, mline+mut_space-mut_size/1.3], lw=1, color='k', zorder=1))
                        if mut.cat == 'Patho':
                            update_disease_acronym_dictionary(mut)
                            plt.text(mstart + mut_size, mline + 0.02 + mut_space, dis_acro, size=7)
        line -= spacing
        if has_muts: line -= spacing * 0.5 # bigger spacing if plotting disease
    line += spacing
    # if muts, plot final disease acronym legend
    for acro, desc in dis_acro_dict.items():
        plt.text(0, line, acro + ' - ' + desc, va='bottom')
        line -= spacing/4.0
