...
fig, axs = plt.subplots(nrows=1, ncols=5, sharey=True, gridspec_kw=gs_kw)
...
axs[4].set_title("Full DBD + flanks\nin alternative isoform", fontsize=10)
sns.swarmplot(
    data=df,
    y="delta_pdi_trunc",
    x="dbd_or_flank_affected",
...
norm = plt.Normalize(
    df.loc[:, "f_disorder_difference"].min(), df.loc[:, "f_disorder_difference"].max()
)
...
fig, axs = plt.subplots(nrows=1, ncols=4, sharey=True, gridspec_kw=gs_kw)
...
axs[1].annotate(
    "ZIC3-1",
    xy=(
        df.loc[(df["alt_iso"] == "ZIC3-1"), "dbd_pct_lost"].values,
...
axs[1].annotate(
    "ZIC3-3",
    xy=(
        df.loc[(df["alt_iso"] == "ZIC3-3"), "dbd_pct_lost"].values,
...
axs[2].scatter(
    ...
    s=point_size**2,
    c=df.loc[
        (df["dbd_pct_lost"] == 0) & (df["dbd_insertion_n_aa"] > 0), "color"
    ].values,
    linewidth=1,
    edgecolor="black",
    clip_on=False,
...
axs[2].annotate(
    "HEY1-1",
    xy=(
        df.loc[(df["alt_iso"] == "HEY1-1"), "dbd_insertion_n_aa"].values,
...
axs[3].annotate(
    "TBX5-2",
    xy=(
        df.loc[(df["alt_iso"] == "TBX5-2"), "dbd_n_aa_to_change"].values,
...
axs[3].annotate(
    "TBX5-3",
    xy=(
        df.loc[(df["alt_iso"] == "TBX5-3"), "dbd_n_aa_to_change"].values,
...
axs[3].annotate(
    "CREB1-1",
    xy=(
        df.loc[(df["alt_iso"] == "CREB1-1"), "dbd_n_aa_to_change"].values,
...
# mirror figure
gs_kw = dict(width_ratios=[0.7, 1, 0.35, 1.5])
fig2, axs2 = plt.subplots(nrows=1, ncols=4, sharey=True, gridspec_kw=gs_kw)
fig2.set_size_inches(w=7.5, h=2)
map1 = axs2[3].imshow(np.stack([t, t]), cmap="flare")
fig.colorbar(
    map1, ax=axs[3], aspect=40, label="% alt. iso. seq. diff.\nin disordered regions"
)
...
fig.savefig(
    "../figures/DBD_or_flank_change_vs_PDI_composite_alt_with_distance_colored_annotated.pdf",
    bbox_inches="tight",
)
