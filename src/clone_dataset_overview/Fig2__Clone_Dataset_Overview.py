...
fig.savefig(
    "../../figures/fig2/clone_collection_families.all.pdf",
    dpi="figure",
    bbox_inches="tight",
)
...
all_3 = (
    set(m1h[m1h["M1H_mean"].abs() > 1].gene_symbol.unique())
    .intersection(set(y2h[y2h["Y2H_result"] == True].ad_gene_symbol.unique()))
    .intersection(set(y1h[y1h["any_true"] > 0].gene_symbol.unique()))
)
all_3
...
iso_sub["at_least_two_fold_activation"] = iso_sub["clone_acc"].map(
    m1h.drop(columns=["gene_symbol"]).set_index("clone_acc").mean(axis=1).abs() > 1
)
...
for i, row in df.iterrows():
    ...
    try:
        ...
        print(
            "not found: clone: %s | bait: %s | orig call: %s"
            % (clone, bait, orig_y1h_call)
        )
...
fig, ax = plt.subplots(1, 1)
fig.set_size_inches(w=1.5, h=2.0)
sns.stripplot(
    data=df,
    x="updated_y1h_call",
    y="Log2(FC)",
...
fig.savefig(
    "../../figures/fig2/PDI-luciferase_validation_point-plot.pdf", bbox_inches="tight"
)
...
validation_titration_plot(
    data=df_nn,
    selections=[df_nn["updated_y1h_call"], ~df_nn["updated_y1h_call"]],
...
fig.savefig(
    "../../figures/fig2/PDI-luciferase_validation_titration-plot.pdf",
    bbox_inches="tight",
)
...
def developmental_tissue_expression_plot(
    ...
    plt.savefig(
        "../../figures/fig2/expression_" + gene_name + "_" + fig_suffix + ".pdf",
        bbox_inches="tight",
    )
...
developmental_tissue_expression_plot(
    "ZNF414", (7.2, 1.75), (0, 6), means_dev, notestis_cols, "means_dev_notestis_large"
)
