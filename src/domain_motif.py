...
ppi["ad_iso_id"] = ppi["ad_clone_acc"].apply(
...
def matching_DDIs(row):
    if pd.isnull(row["partner_domains"]) or pd.isnull(row["tf_domains"]):
        return np.nan
    if len(row["partner_domains"]) == 0 or len(row["tf_domains"]) == 0:
        return np.nan
    matches = ddi.loc[
        (
            ddi["pfam_a"].isin(row["partner_domains"])
            & ddi["pfam_b"].isin(row["tf_domains"])
        )
        | (
            ddi["pfam_a"].isin(row["tf_domains"])
            & ddi["pfam_b"].isin(row["partner_domains"])
        )
    ].values
    if len(matches) == 0:
        return np.nan
...
df = pd.DataFrame(
    data={
        tuple(sorted(p))
        for ps in ggi.loc[ggi["matching_DDI"].notnull(), "matching_DDI"].unique()
        for p in ps
    },
    columns=["pfam_a", "pfam_b"],
)
df["name_a"] = df["pfam_a"].map(
    pfam.loc[:, ["pfam_accession", "domain_name"]]
    .drop_duplicates()
    .set_index("pfam_accession")["domain_name"]
)
df["name_b"] = df["pfam_b"].map(
    pfam.loc[:, ["pfam_accession", "domain_name"]]
    .drop_duplicates()
    .set_index("pfam_accession")["domain_name"]
)
df["description_a"] = df["pfam_a"].map(
    pfam.loc[:, ["pfam_accession", "domain_description"]]
    .drop_duplicates()
    .set_index("pfam_accession")["domain_description"]
)
df["description_b"] = df["pfam_b"].map(
    pfam.loc[:, ["pfam_accession", "domain_description"]]
    .drop_duplicates()
    .set_index("pfam_accession")["domain_description"]
)
df.sort_values(["name_a", "name_b"]).to_csv(
    "../data/internal/DDI_for_manual_annotation.tsv", index=False, sep="\t"
)
...
valid_ddi_pairs = {
    frozenset(p)
    for p in ddi_annot.loc[ddi_annot["to_use"], ["pfam_a", "pfam_b"]].values
}
...
ggi["matching_DDI_filtered"] = ggi["matching_DDI_filtered"].apply(
    lambda x: frozenset(ddi_to_merge.get(tuple(sorted(p)), tuple(sorted(p))) for p in x)
    if pd.notnull(x)
    else x
)
...
pfam_name[
    "PF17725"
] = "YBD"  # one missing from that table because it was added more recently
ggi["DDI"] = ggi["matching_DDI_filtered"].apply(
    lambda x: " and ".join(pfam_name[a] + "|" + pfam_name[b] for a, b in x)
    if pd.notnull(x)
    else x
)
...
ggi["is_same_family_tf"] = ggi["ad_gene_symbol"].map(fam) == ggi["db_gene_symbol"].map(
    fam
)

dlm = pd.read_csv("../data/external/elm_interaction_domains.tsv", sep="\t")
elm = pd.read_csv("../data/external/elm_classes.tsv", header=0, comment="#", sep="\t")
orfs_with_slim_dom = pfam.loc[
    pfam["pfam_accession"].isin(dlm["Interaction Domain Id"]), "orf_id"
].unique()
partner_slim_binding_doms = {
    orf_id: frozenset(
        [
            d
            for d in pfam.loc[pfam["orf_id"] == orf_id, "pfam_accession"].unique()
...
print(
    ggi["putative_DMI"].sum(),
    ggi["putative_DMI"].mean(),
    (ggi["DDI"].isnull() & ggi["putative_DMI"]).sum(),
)

candidates = (
    ggi.loc[ggi["putative_DMI"], :]
...
candidates["names"] = (
    ggi.loc[ggi["putative_DMI"], :]
    .groupby(["db_gene_symbol", "slim_binding_domains_in_partner"])["ad_gene_symbol"]
...
tfs_with_slim_binding_doms = {
    tf_gene_symbol: frozenset(d for d in doms if d in slim_binding_doms)
    for tf_gene_symbol, doms in tf_pfam_domains.items()
...
ggi["slim_binding_domains_in_tf"] = ggi["ad_gene_symbol"].map(
    tfs_with_slim_binding_doms
)
print(
    ggi["slim_binding_domains_in_tf"].notnull().sum(),
    "gene-level PPI where TF has slim bindig domain",
)
...
ggi.loc[
    ggi["slim_binding_domains_in_tf"] == frozenset(["PF00104"]), "ad_gene_symbol"
].value_counts()
...
ggi.loc[
    ggi["slim_binding_domains_in_tf"] == frozenset(["PF00104"]), "db_gene_symbol"
].value_counts()
...
ax.set_ylabel("")
fig.savefig("../figures/DDI-types_pie.pdf", bbox_inches="tight")
...
positive_in_ref = df.loc[
    df["ad_clone_acc"].isin(ref_isos) & (df["Y2H_result"] == True), "gene_level_pair"
].unique()
df = df.loc[
    df["gene_level_pair"].isin(positive_in_ref) & ~df["ad_clone_acc"].isin(ref_isos), :
]
df["matching_DDI"] = df["matching_DDI"].apply(filter_ddi)
df["matching_DDI"] = df["matching_DDI"].apply(
    lambda x: frozenset(ddi_to_merge.get(tuple(sorted(p)), tuple(sorted(p))) for p in x)
    if pd.notnull(x)
    else x
)
...
df["tf_domains"] = df["tf_domains"].apply(
    lambda x: {merged_domains.get(d, d) for d in x}
)
...
def filter_for_domain_in_cloned_reference_isoform(row):
    in_ref = {
        d.accession
        for d in tfs[row["ad_gene_symbol"]].cloned_reference_isoform.aa_seq_features
    }
...
def fraction_of_DDI_domains_removed(row):
    ds = dom.loc[
        (dom["alt_iso"] == row["ad_iso_id"]) & dom["accession"].isin(row["tf_domains"]),
        :,
    ]
    if ds.shape[0] == 0:
        print(row)
...
df["fraction_of_DDI_domains_removed"] = df.apply(
    fraction_of_DDI_domains_removed, axis=1
)
...
data = (
    df.loc[df["Y2H_result"].notnull(), :]
    .groupby(["ad_iso_id", "tf_domains"])["fraction_of_DDI_domains_removed"]
    .mean()
    .to_frame()
)
...
data["Y2H_result_mean"] = nonan.groupby(["ad_iso_id", "tf_domains"])[
    "Y2H_result"
].mean()
data["insertion_in_DDI_domains"] = nonan.groupby(["ad_iso_id", "tf_domains"])[
    "insertion_in_DDI_domains"
].mean()
...
def get_dist(row):
    ...
    return dist.loc[
        (dist["alt_iso"] == alt_iso) & (dist["accession"].isin(dom_accessions)),
...
axs[0].set_title("Full loss of domain", fontsize=10)
sns.swarmplot(
    data=data.loc[data["fraction_of_DDI_domains_removed"] == 1, :],
    y="Y2H_result_mean",
...
axs[1].set_title("Partial loss of domain", fontsize=10)
partial_loss = (data["fraction_of_DDI_domains_removed"] > 0) & (
    data["fraction_of_DDI_domains_removed"] < 1
)
axs[1].scatter(
    data.loc[partial_loss, "fraction_of_DDI_domains_removed"].values,
...
axs[2].scatter(
    data.loc[
        (data["fraction_of_DDI_domains_removed"] == 0), "domain_n_aa_to_change"
    ].values,
...
(ggi.loc[ggi["is_tf_tf"], "is_same_family_tf"].value_counts().plot.pie())
...
ggi.loc[
    ggi["db_gene_symbol"].isin(
        cats.loc[cats["category"] == "nuclear transport", "partner"].values
    )
]
...
ax.set_ylabel("")
fig.savefig("../figures/PPIs-gene-level-manual-categories_pie.pdf", bbox_inches="tight")


# In[39]:


ggi["db_gene_symbol"].map(
    cats.drop_duplicates("partner").set_index("partner")["category"]
).value_counts()
...
ys = np.array(
    [
        len(ggi[ggi["partner_category"] == "TF"]),
        len(ggi[ggi["partner_category"] == "cofactor"]),
...
fig.savefig(
    "../figures/PPIs-gene-level-manual-categories_simplified.pdf",
    dpi="figure",
    bbox_inches="tight",
)
...
fig.savefig(
    "../figures/PPIs-gene-level-manual-categories_cofactors.pdf",
    dpi="figure",
    bbox_inches="tight",
)
...
rbp = pd.read_csv(
    "../data/external/RBPDB_v1.3.1_proteins_human_2012-11-21.tdt", sep="\t", header=None
)
...
kinase_genes = {
    r.description.split()[2][len("gene=") :]
    for r in SeqIO.parse("../data/external/human_kinases.fa", "fasta")
}
...
ggi.loc[
    ggi["matching_DDI"].apply(
        lambda x: not pd.isnull(x) and ("PF00023", "PF16179") in x
    )
]
...
print(
    ppi.loc[
        ppi["db_orf_id"].isin(orfs_with_slim_dom), ["ad_gene_symbol", "db_gene_symbol"]
    ]
    .drop_duplicates()
    .shape[0],
...
dom_slim_ppi = set(
    ppi.loc[ppi["db_orf_id"].isin(orfs_with_slim_dom), "gene_level_pair"].unique()
)
...
def match_elm_motifs(orfs, elm):
    ...
    motifs = pd.DataFrame(
        data=motifs, columns=("orf_id", "region_id", "ELM_ID", "start", "end")
    )
...
def ppi_tf_gene(data, gene_name):
    tf = data.loc[
        ...
        ["ad_clone_name", "db_gene_symbol", "Y2H_result"],
    ].copy()
    tf = tf.pivot(index="ad_clone_name", columns="db_gene_symbol", values="Y2H_result")
    return tf
...
def ppi_linked_isoform_specific_regions(ppi_data, gene):
    ...
    """
    isr = isoform_specific_regions(
        gene, subset={iso.name for iso in gene.cloned_isoforms}
    )
    ppi = ppi_tf_gene(ppi_data, gene.name)
    ppi_isr = {}
    ppi_iso = {
        partner: set(ppi.index[ppi[partner]])
...
slim_binding_domains = pd.merge(
    pfam, dlm, how="inner", left_on="pfam_accession", right_on="Interaction Domain Id"
)
slim_binding_domains = pd.merge(
    slim_binding_domains, ppi, how="inner", left_on="orf_id", right_on="db_orf_id"
)
slim_ppis = slim_binding_domains.loc[
    :,
    [
        "ad_gene_symbol",
        "db_gene_symbol",
...
slim_ppis = slim_ppis.loc[
    slim_ppis.apply(
        lambda x: x["db_gene_symbol"] in ppi_isr[x["ad_gene_symbol"]], axis=1
    ),
    :,
]
slim_ppis["aa_seq_isr"] = slim_ppis.apply(
    lambda x: ppi_isr[x["ad_gene_symbol"]][x["db_gene_symbol"]][1], axis=1
)
...
def isr_contains_slim(row):
    ...
    if row["ELM identifier"] not in elm["ELMIdentifier"].values:
        ...
        return False
    regex = (
        elm.loc[elm["ELMIdentifier"] == row["ELM identifier"], "Regex"]
        .values[0]
        .replace("(", "(?:")
    )
...
slim_ppis.loc[
    slim_ppis["slim_match"] & (slim_ppis["db_gene_symbol"] == "TXK")
].sort_values("ad_gene_symbol")
...
slim_ppis["aa_seq_ref_iso"] = slim_ppis["ad_gene_symbol"].apply(
    lambda x: tfs[x].isoforms[0].aa_seq
)
slim_ppis["regex"] = slim_ppis["ELM identifier"].map(
    elm[["ELMIdentifier", "Regex"]]
    .drop_duplicates()
    .set_index("ELMIdentifier")["Regex"]
    .str.replace("(", "(?:")
)
...
slim_ppis["match_count_ref_iso"] = slim_ppis.apply(
    lambda x: len(re.findall(x["regex"], x["aa_seq_ref_iso"])), axis=1
)
slim_ppis["match_count_isr"] = slim_ppis.apply(
    lambda x: sum(len(re.findall(x["regex"], s)) for s in x["aa_seq_isr"]), axis=1
)
...
slim_ppis["motif_probability"] = slim_ppis["ELM identifier"].map(
    elm.set_index("ELMIdentifier")["Probability"]
)
...
slim_ppis.groupby(["ad_gene_symbol", "db_gene_symbol"])["match_count_isr"].sum().head(
    30
)
...
(
    slim_ppis.groupby(["ad_gene_symbol", "db_gene_symbol"])["match_count_isr"].sum() > 0
).value_counts()
...
gg = (
    slim_ppis.loc[:, ["ad_gene_symbol", "db_gene_symbol", "aa_seq_ref_iso", "isr_len"]]
    .drop_duplicates()
    .copy()
    .set_index(["ad_gene_symbol", "db_gene_symbol"])
)
...
for __ in range(n_samples):
    ...
    rand_seq = pd.merge(
        rand_seq.rename("rand_aa_seq").reset_index(),
        slim_ppis.loc[:, ["ad_gene_symbol", "db_gene_symbol", "regex"]],
        how="inner",
    )
    rand_seq["match_count_rand_aa_seq"] = rand_seq.apply(
        lambda x: len(re.findall(x["regex"], x["rand_aa_seq"])), axis=1
    )
    null_dist.append(
        (
            rand_seq.groupby(["ad_gene_symbol", "db_gene_symbol"])[
                "match_count_rand_aa_seq"
            ].sum()
            > 0
        ).sum()
    )
...
slim_ppis_spec = (
    slim_ppis.sort_values("motif_probability", ascending=True)
...
gg = (
    slim_ppis_spec.loc[
        :, ["ad_gene_symbol", "db_gene_symbol", "aa_seq_ref_iso", "isr_len"]
    ]
    .drop_duplicates()
    .copy()
    .set_index(["ad_gene_symbol", "db_gene_symbol"])
...
real = (
    slim_ppis_spec.groupby(["ad_gene_symbol", "db_gene_symbol"])[
        "match_count_isr"
    ].sum()
    > 0
).sum()
...
(
    slim_ppis[["ad_gene_symbol", "db_gene_symbol", "isr_frac"]].drop_duplicates()[
        "isr_frac"
    ]
    < 0.2
).sum()
...
(
    slim_ppis.loc[
        :, ["ad_gene_symbol", "db_gene_symbol", "isr_frac"]
    ].drop_duplicates()["isr_frac"]
    * 100
).plot.hist()
...
res = slim_ppis.sort_values("motif_probability", ascending=True).drop_duplicates(
    ["ad_gene_symbol", "db_gene_symbol", "pfam_accession"]
)
print(res["match_count_ref_iso"].sum() / res["aa_seq_len_ref_iso"].sum())
print(res["match_count_isr"].sum() / res["isr_len"].sum())
print(
    (res["match_count_ref_iso"] - res["match_count_isr"]).sum()
    / (res["aa_seq_len_ref_iso"] - res["isr_len"]).sum()
)
print(
    (
        slim_ppis.loc[slim_ppis["slim_match"], ["ad_gene_symbol", "db_gene_symbol"]]
        .drop_duplicates()
        .shape[0]
    )
)
...
(
    ...
    .drop_duplicates(["ad_gene_symbol", "db_gene_symbol", "domain_description"])
).to_csv("../output/slim_matches.csv", index=False)


# In[131]:


match_pairs = slim_ppis.loc[
    slim_ppis["slim_match"], ["ad_gene_symbol", "db_gene_symbol"]
].drop_duplicates()
match_pairs.head()
...
print(
    match_pairs.shape[0],
    "gene-level PPIs corresponding to motif in the isoform regions",
)
print(
    "out of {} possible".format(
        slim_ppis[["ad_gene_symbol", "db_gene_symbol"]].drop_duplicates().shape[0]
    )
)
...
ppi.loc[ppi["db_gene_symbol"] == "PIN1", "ad_gene_symbol"].nunique()
ppi.loc[ppi["db_gene_symbol"] == "PIN1", "ad_gene_symbol"].unique()
...
# smallest region
slim_ppis["isr_length"] = slim_ppis["aa_seq_isr"].apply(
    lambda x: min(len(s) for s in x)
)
(
