...
g.savefig(
    "../../figures/GENCODE_iso_counts_per_family.pdf", bbox_inches="tight", dpi="figure"
)
...
fig.savefig(
    "../../figures/n-isoforms-per-gene_by-1TPM-cutoff_hist-GTEx.pdf",
    bbox_inches="tight",
)
...
for ax in axs:
    ...
    pctax.set_yticks(range(0, 46, 5), minor=True)
fig.savefig(
    "../../figures/n-isoforms-per-gene_by-1TPM-cutoff_hist-GTEx_downsamp.pdf",
    bbox_inches="tight",
)
...
for ax in axs:
    ...
    pctax.set_yticks(range(0, 46, 5), minor=True)
fig.savefig(
    "../../figures/n-isoforms-per-gene_by-1TPM-cutoff_hist-GTEx_dev.pdf",
    bbox_inches="tight",
)
...
p = (
    (means_gtex_downsample.loc[means_gtex_downsample.index.isin(alt_isos), :] >= 1)
    .any(axis=1)
    .sum()
)
...
p = (
    (
        means_gtex_downsample.loc[means_gtex_downsample.index.isin(alt_isos), :]
        >= np.log2(5 + 1)
    )
    .any(axis=1)
    .sum()
...
p = (
    (means_gtex_downsample.loc[means_gtex_downsample.index.isin(ref_isos), :] >= 1)
    .any(axis=1)
    .sum()
)
...
p = (
    (
        means_gtex_downsample.loc[means_gtex_downsample.index.isin(ref_isos), :]
        >= np.log2(5 + 1)
    )
    .any(axis=1)
    .sum()
...
plt.subplots_adjust(wspace=0.4)
fig.savefig(
    "../../figures/downsampled-GTEx-control-max-expression_by-reference-vs-alternative_pie.pdf",
    bbox_inches="tight",
)
...
p = (
    (means_gtex_downsample.loc[means_gtex_downsample.index.isin(alt_isos), cols] >= 1)
    .any(axis=1)
    .sum()
)
n = means_gtex_downsample.index.isin(alt_isos).sum()
...
p = (
    (
        means_gtex_downsample.loc[means_gtex_downsample.index.isin(alt_isos), cols]
        >= np.log2(5 + 1)
    )
    .any(axis=1)
    .sum()
...
p = (
    (means_gtex_downsample.loc[means_gtex_downsample.index.isin(ref_isos), cols] >= 1)
    .any(axis=1)
    .sum()
)
n = means_gtex_downsample.index.isin(ref_isos).sum()
...
p = (
    (
        means_gtex_downsample.loc[means_gtex_downsample.index.isin(ref_isos), cols]
        >= np.log2(5 + 1)
    )
    .any(axis=1)
    .sum()
...
plt.subplots_adjust(wspace=0.4)
fig.savefig(
    "../../figures/downsampled-GTEx-max-expression_by-reference-vs-alternative_no-testis_pie.pdf",
    bbox_inches="tight",
)
...
p = (
    (means_dev.loc[means_dev.index.isin(alt_isos), cols] >= np.log2(5 + 1))
    .any(axis=1)
    .sum()
)
n = means_dev.index.isin(alt_isos).sum()
...
p = (
    (means_dev.loc[means_dev.index.isin(ref_isos), cols] >= np.log2(5 + 1))
    .any(axis=1)
    .sum()
)
n = means_dev.index.isin(ref_isos).sum()
...
plt.subplots_adjust(wspace=0.4)
fig.savefig(
    "../../figures/developmental-max-expression_by-reference-vs-alternative_no-testis_pie.pdf",
    bbox_inches="tight",
)
...
x_max = 11
axs[1].hist(
    means_gtex_downsample.max(axis=1)[means_gtex_downsample.index.isin(alt_isos)],
    bins=n_bins,
    range=(0, x_max),
)
axs[0].hist(
    means_gtex_downsample.max(axis=1)[means_gtex_downsample.index.isin(ref_isos)],
    bins=n_bins,
    range=(0, x_max),
)
...
axs[1].set_xticklabels(x_ticks)
fig.savefig(
    "../../figures/expression_downsampled-GTEX-control_GENCODE-isoforms_by-reference-vs-alternative.pdf",
    bbox_inches="tight",
)
...
n_bins = 100
axs[0].hist(
    f_gtex_downsample.max(axis=1).loc[f_gtex_downsample.index.isin(ref_isos)].values,
    range=(0, 100),
    bins=n_bins,
)
axs[1].hist(
    f_gtex_downsample.max(axis=1).loc[f_gtex_downsample.index.isin(alt_isos)].values,
    range=(0, 100),
    bins=n_bins,
)
axs[1].set_xlabel(
...
fig.savefig(
    "../../figures/expression-scatter-ref_v_alt-gtex-downsample.pdf",
    bbox_inches="tight",
)
...
fig.savefig("../../figures/expression-ratio-scatter-ref-gtex.pdf", bbox_inches="tight")
...
fig.savefig("../../figures/expression-ratio-scatter-alt-gtex.pdf", bbox_inches="tight")
...
n_switches = df[
    (df["min_ratio_gtex_downsample_ref"] < 10)
    & (df["max_ratio_gtex_downsample_ref"] > 90)
]
n_off = df[
    (df["min_ratio_gtex_downsample_ref"] < 10)
    & (df["max_ratio_gtex_downsample_ref"] < 10)
]
...
ax.set_title("n=%s ref isoforms" % len(df))
fig.savefig(
    "../../figures/expression-ratio-scatter-ref-gtex-downsample.pdf",
    bbox_inches="tight",
)
...
n_switches = df[
    (df["min_ratio_gtex_downsample_alt"] < 10)
    & (df["max_ratio_gtex_downsample_alt"] > 90)
]
n_off = df[
    (df["min_ratio_gtex_downsample_alt"] < 10)
    & (df["max_ratio_gtex_downsample_alt"] < 10)
]
print(len(n_switches))
...
fig.savefig(
    "../../figures/expression-ratio-scatter-alt-gtex-downsample.pdf",
    bbox_inches="tight",
)
...
fig.savefig("../../figures/expression-ratio-scatter-ref-dev.pdf", bbox_inches="tight")
...
fig.savefig("../../figures/expression-ratio-scatter-alt-dev.pdf", bbox_inches="tight")
...
ax = gtex_bar.plot.bar(
    x="index", stacked=True, color=palette.values(), figsize=(1.4, 1.4)
)
...
plt.savefig("../../figures/expression-switch-bar-gtex.pdf", bbox_inches="tight")
...
plt.savefig("../../figures/expression-switch-bar-dev.pdf", bbox_inches="tight")
...
tmp_nn = tmp[
    (~pd.isnull(tmp["min_ratio_dev_alt"])) & (~pd.isnull(tmp["max_ratio_dev_alt"]))
]
tmp_srt = tmp_nn[
    (tmp_nn["max_ratio_dev_alt"] > 90) & (tmp_nn["min_ratio_dev_alt"] < 10)
]
tmp_srt[
    [
        "gene",
...
].sort_values(by="max_ratio_dev_alt", ascending=False).head(20)
...
developmental_tissue_expression_plot(
    "TBX5", "Set2", (6, 1.75), (0, 6), means_gtex, gtex_cols, "means_gtex"
)
...
ss_alt = len(
    ref_alt_map_nonan[
        (ref_alt_map_nonan["max_ratio_gtex_downsample_alt"] >= 10)
    ].gene.unique()
)
ss_alt
...
ss_alt = len(
    ref_alt_map_nonan[(ref_alt_map_nonan["max_ratio_dev_alt"] >= 10)].gene.unique()
)
ss_alt
...
ax = sns.lineplot(
    data=genc_melt[~genc_melt["variable"].isin(["n_tx", "n_genes"])],
    x="release",
    y="value",
    hue="variable",
)
