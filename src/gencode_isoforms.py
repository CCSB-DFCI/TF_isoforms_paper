...
fig.savefig(
    "../figures/n-isoforms-per-gene_GENCODE-v30-TFs_hist.pdf", bbox_inches="tight"
)
...
pctax = ax.secondary_yaxis(
    "right",
    functions=(
        functools.partial(num2pct, total=n),
...
p = min(
    (
        stats.mannwhitneyu(
            df.loc[df["gene"].isin(non_nr_ligand_tfs), "n_isoforms"].values,
            df.loc[~df["gene"].isin(non_nr_ligand_tfs), "n_isoforms"].values,
        ).pvalue
    ),
    1,
...
fig.savefig(
    "../figures/n-isoforms-per-gene_GENCODE-v30-non-NR-receptor-TFs_hist.pdf",
    bbox_inches="tight",
)
...
df = pd.DataFrame(
    [(tf.name, len(tf.isoforms), tf.tf_family) for name, tf in tfs.items()],
...
df.loc[
    df["family"].isin(small_families) | (df["family"] == "Unknown"), "family_merged"
] = "other"
df.head()
...
for fam, ax in zip(families, axs):
    ...
    p = min(
        (
            stats.mannwhitneyu(
                df.loc[df["family_merged"] == fam, "n_isoforms"].values,
                df.loc[df["family_merged"] != fam, "n_isoforms"].values,
            ).pvalue
            * len(families)
        ),
...
fig.savefig(
    "../figures/n-isoforms-per-gene_GENCODE-v30-TFs_by-family_hist.pdf",
    bbox_inches="tight",
)
...
cats = pd.DataFrame(
    [
        tf.splicing_categories(tf.MANE_select_isoform.name, alt_iso.name)
        for tf in tfs.values()
        for alt_iso in tf.isoforms[1:]
        if tf.has_MANE_select_isoform and alt_iso != tf.MANE_select_isoform
    ]
)
...
ax.set_yticks(range(0, int(ax.get_ylim()[1]), 5), minor=True)
fig.savefig(
    "../figures/splice-categories_TFs-in-GENCODE-v30_bar.pdf", bbox_inches="tight"
)
...
plt.savefig("../figures/isoform-categories_gencode_UpSet-plot.pdf", bbox_inches="tight")
...
df["n_isoforms_cloned"] = (
    df["gene"]
    .map({tf.name: len(tf.isoforms) for tf in tfs_cloned.values()})
    .fillna(0)
    .astype(int)
)
...
for fam, ax in zip(families, axs):
    ...
    p = min(
        (
            stats.mannwhitneyu(
                df.loc[df["family_merged"] == fam, "n_isoforms_cloned"].values,
                df.loc[df["family_merged"] != fam, "n_isoforms_cloned"].values,
            ).pvalue
            * len(families)
        ),
...
fig.savefig(
    "../figures/n-isoforms-per-gene_TFiso1-TFs_by-family_hist.pdf", bbox_inches="tight"
)
...
ax.bar(
    x=xs,
    height=[df["n_isoforms_cloned"].value_counts().to_dict().get(x, 0) for x in xs],
)
ax.set_xticks(xs)
for pos in ["top", "right", "bottom"]:
    ax.spines[pos].set_visible(False)
...
ax.text(s=f'mean = {df["n_isoforms_cloned"].mean():.2f} isoforms', x=10, y=0.5 * n)
...
pctax.set_yticks(range(0, 46, 5), minor=True)
pctax.set_yticklabels([f"{y:.0f}%" for y in pctax.get_yticks()])
fig.savefig("../figures/n-isoforms-per-gene_TFiso1-TFs_hist.pdf", bbox_inches="tight")
