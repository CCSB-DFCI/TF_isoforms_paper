...
iso_pairs["both_iso_y2h_pos"] = iso_pairs["clone_acc_ref"].isin(
    iso_gte_1_pos_ppi_iso_data_only
) & iso_pairs["clone_acc_alt"].isin(iso_gte_1_pos_ppi_iso_data_only)
iso_pairs["both_iso_y2h_pos_all_data"] = iso_pairs["clone_acc_ref"].isin(
...
clone_to_orf_id = (
    y2h[["ad_clone_acc", "ad_orf_id"]]
    .drop_duplicates()
    .set_index("ad_clone_acc")["ad_orf_id"]
)
...
non_zero_iso = set(
    iso_pairs.loc[
        (iso_pairs["n_PPI_successfully_tested_in_ref_and_alt"] >= 2)
...
n2h.loc[
    (n2h["test_orf_ida"].isin(tcf4_orf_ids) | n2h["test_orf_idb"].isin(tcf4_orf_ids))
    & (n2h["source"] == "pos"),
    "result",
].value_counts()
...
mismatch = (
    n2h["source"].isin({"pos", "neg", "pos-matched-neg"}) & n2h["Y2H_result"].isnull()
)
mismatch = mismatch | (n2h["source"].isin({"pos"}) & (n2h["Y2H_result"] != True))
mismatch = mismatch | (
    n2h["source"].isin({"neg", "pos-matched-neg"}) & (n2h["Y2H_result"] != False)
)
...
samples = {
    ...
    "pos_non_zero_iso": (n2h["source"] == "pos") & n2h["non_zero_iso"],
    "pos-matched-neg_non_zero_iso": (n2h["source"] == "pos-matched-neg")
    & n2h["non_zero_iso"],
    "neg_non_zero_iso": (n2h["source"] == "neg") & n2h["non_zero_iso"],
    " ": pd.Series(index=n2h.index, data=False),
    "pos_other": (n2h["source"] == "pos") & ~n2h["non_zero_iso"],
    "pos-matched-neg_other": (n2h["source"] == "pos-matched-neg")
    & ~n2h["non_zero_iso"],
...
(
    n2h.loc[
        n2h["source"].isin({"pos", "neg", "pos-matched-neg"}) & n2h["non_zero_iso"],
        [
            "ad_gene_symbol",
            "ad_clone_acc",
            "db_gene_symbol",
    ...
    .to_csv("../output/N2H_non_zero_iso_only.tsv", index=False, sep="\t")
)
...
for cat, ax in zip(n2h["source"].unique(), axes):
    ...
    is_neg = n2h["result"] == 0
    ax.scatter(
        x=np.log2(n2h.loc[is_cat & is_neg, "max_control"]),
        y=np.log2(n2h.loc[is_cat & is_neg, "score"]),
    )
    ax.scatter(
        x=np.log2(n2h.loc[is_cat & is_pos, "max_control"]),
...
n2h.loc[n2h["source"].isin(["TF_RRS", "litbm"]), "iso_orf_id"].map(
    clone_to_orf_id.reset_index().set_index("ad_orf_id")["ad_clone_acc"]
).unique()
