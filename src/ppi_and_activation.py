...
cofactor_partners = set(
    cats.loc[cats["category"] == "cofactor", "gene_symbol_partner"].unique()
)
coactivator_partners = set(
    cats.loc[cats["cofactor_type"] == "coactivator", "gene_symbol_partner"].unique()
)
corepressor_partners = set(
    cats.loc[cats["cofactor_type"] == "corepressor", "gene_symbol_partner"].unique()
)
...
df = pd.DataFrame(
    data=data, columns=["tf_gene", "gene_symbol_partner", "activation_diff"]
).dropna()
df["num_points"] = df["gene_symbol_partner"].map(
    df["gene_symbol_partner"].value_counts()
)
df = df.sort_values(
    ["num_points", "gene_symbol_partner", "activation_diff"], ascending=False
)
