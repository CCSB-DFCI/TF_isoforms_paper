...
tf_datalist = "\n".join(
    '        <option value="{}"/>'.format(name) for name in sorted(tfs.keys())
)
...
for tf in tfs.values():
    ...
    for fmt in [".svg"]:
        plt.savefig(
            shared_fig_dir / "{}_y2h-profile{}".format(tf.name, fmt),
            bbox_inches="tight",
        )
    plt.close(plt.gcf())
...
for tf in tfs.values():
    ...
    for fmt in [".svg"]:
        plt.savefig(
            shared_fig_dir / "{}_m1h-profile{}".format(tf.name, fmt),
            bbox_inches="tight",
        )
    plt.close(plt.gcf())
...
for gene_name in isoforms["gene"].unique():
    ...
    for fmt in [".svg"]:
        plt.savefig(
            shared_fig_dir / "{}_y1h-profile{}".format(gene_name, fmt),
            bbox_inches="tight",
        )
    plt.close(plt.gcf())
...
for tf in tfs.values():
    ...
    for fmt in [".svg"]:
        plt.savefig(
            shared_fig_dir
            / "{}_cloned-isoforms_protein-diagram{}".format(tf.name, fmt),
            bbox_inches="tight",
        )
    plt.close(plt.gcf())
...
for tf in tfs.values():
    ...
    for fmt in [".svg"]:
        plt.savefig(
            shared_fig_dir
            / "{}_cloned-plus-ensembl-isoforms_exon-diagram{}".format(tf.name, fmt),
            bbox_inches="tight",
        )
    plt.close(plt.gcf())
